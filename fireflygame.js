...
function mouseClicked() {
  if (onFly) {
    freeFireflies.splice(currentFreeFly, 1);
    if (bugsInJar.length < 20) {
...
let Text = function(string, x, y, a) {
  this.string = string;
  this.x = x;
  this.y = y;
  this.a = a;
...
class Window {
  constructor(x, y, width, height, isLightOn, aGlass) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
  ...
  stainedGlassWindow() {
    ...
    stroke('#696969');
    strokeWeight(4);
    fill(255, 109, 124, this.aGlass);
    rect(this.x, this.y, this.width, this.height);
    noStroke();
    fill(0, 0, 255, this.aGlass);
  ...
  onGlassClick() {
    if (this.onWindow && this.aGlass === 255) {
      this.aGlass = 100;
    } else if (this.onWindow && this.aGlass === 100) {
      this.aGlass = 255;
    }
  }
...
let Star = function(x, y, dMIN, dMAX, d) {
  this.x = x;
  this.y = y;
...
Star.prototype.shine = function() {
  this.dMIN += 2;
  this.d = ((sin(radians(this.dMIN)) + 1.5) / 2) * this.dMAX;
}
...
let Firefly = function(position, value, a) {
...
Firefly.prototype.shine = function() {
  this.value += 1;
  this.a = ((sin(radians(this.value)) + 1) / 2) * 255;
}
...
function freeFirefly(position, value, s, onFly) {
